...
const useInventoryData = (url: string) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "Failed to fetch inventory data, loading mock data instead.",
        e,
      );
    } finally {
      setLoading(false);
...
const App: React.FC = () => {
  ...
  const handleSaveSettings = (newUrl: string) => {
    ...
    localStorage.setItem(LOCAL_STORAGE_URL_KEY, newUrl);
  ...
  return (
    <div className="min-h-screen bg-[#E5DCC5] dark:bg-[#0F0A09] text-[#4A1E1E] dark:text-[#EAEAEA] flex flex-col items-center p-4 sm:p-6 md:p-12 transition-colors duration-500 font-serif">
      <header className="w-full max-w-7xl mb-8">
        <div className="flex justify-between items-center">
          <div className="flex flex-col sm:flex-row sm:items-baseline sm:space-x-6">
            <h1 className="sr-only">{locales.headerTitle[language]}</h1>
          ...
          <div className="flex items-center space-x-2">
            ...
            <ThemeToggle
              ...
              labels={{
                light: locales.themeToggle.light[language],
                dark: locales.themeToggle.dark[language],
              }}
            />
          </div>
        </div>
      ...
      <main className="w-full max-w-7xl">
        ...
        {error && (
          <p className="text-center text-red-500 my-4 p-4 bg-red-100 dark:bg-red-900/30 rounded-lg">{`${locales.dataSourceUrl[language]}からのデータ取得に失敗しました: ${error}`}</p>
        )}
